...
def load_kernel(args):
    ...
    if args.kernel == "binomial":
        ...
        kernel = kernel / np.sum(kernel) # properly normalizing filter
...
def Downsampling(args, I, kernel):
    ...
    """
    # Given an image I, convolve it with a 2D binomial filter to obtain I_convolved 
    I_convolved = Convolution(args, I, kernel)
    # Subsample I_convolved by a factor of two: 
    I_out = I_convolved[::2, ::2, :]
    return I_out
...
def Upsampling(args, I, kernel):
    ...
    # First double its size in each dimension and multiply the pixel values by 4 
    I_out[::2, ::2, :] = 4 * I 
    # Convolve with a 2D binomial filter 
    I_out = Convolution(args, I_out, kernel)
...
def Laplacian_pyramid(args, G_pyramid, kernel, scale):
    ...
    for n in range(scale):
        # Create a sequence of residual images: 
        upsample = Upsampling(args, G_pyramid[n+1], kernel)
        Output[n] = G_pyramid[n] - upsample 
...
def main(args):
    ...
    if int(args.current_step) >= 3:
        ...
        ## Sanity check
        if np.sum((I - I_reconstruct) ** 2) > (10 ** (-12)):
